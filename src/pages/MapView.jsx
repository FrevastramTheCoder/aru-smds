...
const MapView = () => {
  ...
  const SPATIAL_API_BASE = 'https://smds.onrender.com/api/spatial';
  ...
  useEffect(() => {
    ...
    const type = category ? (Object.entries({
      ...
      'recreational-areas': 'recreational_areas',
      'aru-boundary': 'aru_boundary',
    }).find(([k]) => k === category)?.[1] || 'buildings') : 'buildings';
      ...
      stats[layer] = {
        ...
        properties: features.reduce((acc, feature) => {
          if (feature.properties) {
            Object.entries(feature.properties).forEach(([key, value]) => {
              if (!acc[key]) acc[key] = new Set();
              if (value !== null && value !== undefined) acc[key].add(value.toString());
            });
          }
          return acc;
        }, {}),
      ...
      try {
        ...
        for (const layer of layers) {
          ...
          newSpatialData[layer] = features.map(feature =>
            feature.geometry?.type === 'Polygon' ? ensurePolygonWindingOrder(feature) : feature
          );
          spatialDataCache.current.set(cacheKey, newSpatialData[layer]);
          setFailedLayers(prev => new Set([...prev].filter(l => l !== layer)));
        }
  ...
  const ensurePolygonWindingOrder = (feature) => {
    ...
    try {
      ...
      if (coordinates[0]?.length >= 3) {
        const area = calculatePolygonArea(coordinates[0]);
        if (area > 0) coordinates[0] = coordinates[0].reverse();
      }
      for (let i = 1; i < coordinates.length; i++) {
        if (coordinates[i].length >= 3) {
          const area = calculatePolygonArea(coordinates[i]);
          if (area < 0) coordinates[i] = coordinates[i].reverse();
        }
      }
      ...
      try {
        ...
        for (const layer of selectedLayers) {
          ...
          try {
            ...
            const fc = resp.data || { type: 'FeatureCollection', features: [] };
            newSpatialData[layer] = (Array.isArray(fc.features) ? fc.features : []).map(feature =>
              feature.geometry?.type === 'Polygon' ? ensurePolygonWindingOrder(feature) : feature
            );
            setFailedLayers(prev => new Set([...prev].filter(l => l !== layer)));
          } catch (err) {
  ...
  }, [selectedLayers, navigate, availableEndpoints]);
  ...
  const handleFilterChange = (layer, property, value, checked) => {
    setActiveFilters(prev => {
      ...
      if (checked) {
        ...
        if (newFilters[property]) {
          newFilters[property] = newFilters[property].filter(v => v !== value);
          if (newFilters[property].length === 0) delete newFilters[property];
        }
      }
      return newFilters;
    });
  ...
  // Bounds change handler
  const handleBoundsChange = (bounds) => {
    if (selectedLayers.size > 0) fetchGeoByBbox(selectedLayers, bounds, 0.00001);
  };
  ...
  const retryFailedLayers = () => {
    if (failedLayers.size === 0) return;
    setSelectedLayers(prev => new Set([...prev, ...failedLayers]));
  };
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden">
      <div className="w-80 bg-gradient-to-b from-[#2c3e50] to-[#1a2530] text-white flex flex-col shadow-xl z-[1000] overflow-y-auto sidebar">
        <div className="p-5 text-center bg-[#1a2530] border-b border-[#34495e]">
          <h1 className="text-xl mb-1 text-[#3498db]">
            <i className="fas fa-map mr-2"></i>GeoMap Manager
          </h1>
          <p className="text-sm text-[#ecf0f1]">Advanced mapping interface</p>
        </div>

        <div className="p-3 bg-[#2c3e50] border-b border-[#34495e]">
          <input
            type="text"
            placeholder="Search features..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full p-2 rounded-full bg-[#1a2530] text-white border-none focus:outline-none"
          />
        ...
        {error && (
          <div className="p-2 bg-[#ffebee] border border-[#f44336] rounded mx-3 mb-3">
            <p className="text-[#d32f2f] m-0">{error}</p>
          </div>
        )}

        {failedLayers.size > 0 && (
          <div className="p-2 bg-[#fff3cd] border border-[#ffeaa7] rounded mx-3 mb-3">
            <p className="text-[#856404] mb-2">Failed to load: {Array.from(failedLayers).join(', ')}</p>
            <button
              onClick={retryFailedLayers}
              className="px-2 py-1 bg-[#ffc107] text-black rounded text-xs"
            >
              Retry Failed Layers
            </button>
          </div>
        ...
        <div className="p-3">
          <button
            onClick={() => setShowFilters(!showFilters)}
            className="w-full p-2 bg-[#6c757d] text-white rounded mb-3"
          >
            {showFilters ? 'Hide Filters' : 'Show Filters'}
          </button>

          {showFilters && Object.keys(activeFilters).length > 0 && (
            <div className="mb-3 p-2 bg-[#e9ecef] rounded">
              <div className="flex justify-between items-center">
                <strong>Active Filters:</strong>
                <button
                  onClick={clearFilters}
                  className="px-2 py-1 bg-[#dc3545] text-white rounded text-xs"
                >
                  Clear All
                </button>
              </div>
          ...
          <div className="mb-3">
            <div
              className="flex justify-between items-center p-2 bg-[#2c3e50] rounded cursor-pointer"
              onClick={() => toggleSection('landbase')}
            >
              <h3 className="flex items-center m-0 text-base">
                <i className="fas fa-layer-group mr-2 text-[#3498db]"></i>Landbase Layers
              </h3>
              <i className={`fas fa-chevron-${collapsedSections.landbase ? 'down' : 'up'}`}></i>
            </div>
            {!collapsedSections.landbase && (
                  ...
                  <div key={key} className="mb-2">
                    <label className="flex items-center cursor-pointer">
                      <input
                        ...
                        disabled={availableEndpoints[key] === false}
                      />
                      <div className="w-5 h-5 mr-2 flex justify-center items-center bg-[#2c3e50] rounded">
                        <i className={`fas fa-${icon} text-xs`}></i>
                      </div>
                      <span
                        className={availableEndpoints[key] === false ? 'text-[#999] line-through' : ''}
                      >
                        {label}
                        {availableEndpoints[key] === false && ' (Not Available)'}
                      </span>
                      {loadingLayers.has(key) && <span className="ml-2 text-[#007bff]">⏳</span>}
                      {mapStats[key] && (
                        <span className="ml-2 text-xs text-[#666]">({mapStats[key].count})</span>
                      )}
                    </label>
                    <div className="ml-6 mt-1 flex items-center gap-2">
                      <input
                        ...
                        value={getLayerColor(key)}
                        onChange={(e) => handleColorChange(key, e.target.value)}
                        className="w-8 h-5 p-0 border-none"
                        disabled={availableEndpoints[key] === false}
                      />
                      <button
                        onClick={() => resetColor(key)}
                        className="px-2 py-1 bg-[#6c757d] text-white rounded text-xs disabled:opacity-50"
                        disabled={availableEndpoints[key] === false || !customColors[key]}
                      >
                        Reset
                      </button>
          ...
          <div className="mb-3">
            ...
            >
              <h3 className="flex items-center m-0 text-base">
                <i className="fas fa-globe mr-2 text-[#3498db]"></i>Base Layers
              </h3>
              <i className={`fas fa-chevron-${collapsedSections.base ? 'down' : 'up'}`}></i>
            </div>
            {!collapsedSections.base && (
              <div>
                {[
                  ...
                  <div
                    key={layer.key}
                    className="flex items-center p-2 my-1 bg-[#34495e] rounded cursor-pointer"
                  >
                    <input
                      type="radio"
                      name="baseLayer"
                    ...
                    />
                    <div className="w-5 h-5 mr-2 flex justify-center items-center bg-[#2c3e50] rounded">
                      <i className={`fas fa-${layer.icon} text-xs`}></i>
                    </div>
          ...
          <div className="mb-3">
            ...
            >
              <h3 className="flex items-center m-0 text-base">
                <i className="fas fa-cloud-sun mr-2 text-[#3498db]"></i>Weather Overlays
              </h3>
              <i className={`fas fa-chevron-${collapsedSections.weather ? 'down' : 'up'}`}></i>
            </div>
            {!collapsedSections.weather && (
              <div>
                {[
                  ...
                  >
                    ...
                    <div className="w-5 h-5 mr-2 flex justify-center items-center bg-[#2c3e50] rounded">
                      <i className={`fas fa-${layer.icon} text-xs text-[#3498db]`}></i>
                    </div>
                    <span className="text-sm">{layer.label}</span>
                  </div>
                ))}
          ...
          <div className="mb-3">
            ...
            >
              <h3 className="flex items-center m-0 text-base">
                <i className="fas fa-map-legend mr-2 text-[#3498db]"></i>Legend
              </h3>
              <i className={`fas fa-chevron-${collapsedSections.legend ? 'down' : 'up'}`}></i>
            </div>
            {!collapsedSections.legend && (
              <div className="p-2 bg-[#2c3e50] rounded">
                {Object.entries(defaultLayerColors).map(([layer, color]) => (
                  <div key={layer} className="flex items-center mb-2 p-1">
                    <div className="w-4 h-4 mr-2 rounded border border-black" style={{ backgroundColor: getLayerColor(layer) }}></div>
                    <span className="text-xs">{layer.replace(/_/g, ' ').toUpperCase()}</span>
                    {mapStats[layer] && <span className="ml-2 text-xs text-[#666]">({mapStats[layer].count})</span>}
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="mt-3">
            <h4 className="text-base">Export</h4>
            <button
              className="w-full p-2 bg-[#007bff] text-white rounded mb-2 disabled:opacity-50"
              onClick={() => exportData('geojson')}
              disabled={isExporting}
            >
              {isExporting ? `Exporting... ${exportProgress}%` : 'Export GeoJSON'}
            ...
            <button
              className="w-full p-2 bg-[#28a745] text-white rounded mb-2 disabled:opacity-50"
              onClick={() => exportData('csv')}
              disabled={isExporting}
            >
              Export Statistics CSV
          ...
          <button
            className="w-full p-2 bg-[#28a745] text-white rounded mb-2"
            onClick={() => {
              const allAvailableLayers = dataTypes
                .filter(dt => availableEndpoints[dt.key] !== false)
                .map(dt => dt.key);
          ...
          <button
            className="w-full p-2 bg-[#6c757d] text-white rounded mb-2"
            onClick={() => setSelectedLayers(new Set())}
          >
            Clear All Layers
          </button>
          <button
            className="w-full p-2 bg-[#dc3545] text-white rounded"
            onClick={handleLogout}
          >
            Logout
          </button>
      ...
      <div className="flex-1 relative">
        <MapContainer
          ...
          style={{ width: '100%', height: '100%' }}
          whenReady={(map) => {
            map.target.on('moveend', () => handleBoundsChange(map.target.getBounds()));
          }}
        >
          <LayersControl position="topright">
            <LayersControl.BaseLayer checked name="OpenStreetMap">
              ...
              selectedLayers.has(layer) && (
                <LayersControl.Overlay key={layer} name={layer} checked>
                  <GeoJSON
                    ...
                    style={{ color: getLayerColor(layer), weight: 2, opacity: 0.7, fillOpacity: 0.5 }}
                    onEachFeature={(feature, layer) => {
                      if (feature.properties) {
                        const popupContent = Object.entries(feature.properties)
                          .map(([key, value]) => `<b>${key}:</b> ${value}`)
