...
function MapView() {
  ...
  // fetch by bbox with caching, winding order, retries
  const fetchGeoByBbox = useCallback(
    debounce(async (layers, bounds, simplify = 0.00001) => {
      ...
      const key = `${Array.from(layers).join('-')}-${bounds.toBBoxString()}`;
      if (lastBoundsKeyRef.current === key) return;
      lastBoundsKeyRef.current = key;

      setLoading(true);
      setLoadingLayers(new Set([...layers]));
      const bbox = `${bounds.getWest()},${bounds.getSouth()},${bounds.getEast()},${bounds.getNorth()}`;
      const newSpatialData = { ...spatialData };
      ...
      for (const layer of layers) {
        ...
        try {
          ...
          newSpatialData[layer] = features.map(f => f.geometry?.type === 'Polygon' ? ensurePolygonWindingOrder(f) : f);
          spatialDataCache.current.set(cacheKey, newSpatialData[layer]);
          setFailedLayers(prev => { const s = new Set(prev); s.delete(layer); return s; });
        } catch {
          newSpatialData[layer] = [];
          setFailedLayers(prev => new Set([...prev, layer]));
        }
      ...
      setLoading(false);
      setLoadingLayers(new Set());
    }, 800), [spatialData, availableEndpoints, navigate]
  );
  ...
  const ensurePolygonWindingOrder = (feature) => {
    ...
    try {
      ...
      const area = coords[0].reduce((a, [x0,y0], i, arr) => {
        const [x1,y1] = arr[(i+1)%arr.length]; return a + (x0*y1 - x1*y0);
      }, 0) / 2;
      if (area > 0) coords[0].reverse();
      return { ...feature, geometry: { ...feature.geometry, coordinates: coords } };
    } catch { return feature; }
  };

  // The rest of code remains same as your provided implementation:
  // - Filtering, search, color picking, export, retry failed, logout
  // - MapComponent display

  return (
    <div style={{ display: 'flex', height: '90vh', gap: '16px' }}>
      {/* LEFT PANEL (controls, filters, export) */}
      {/* ... your existing left panel code ... */}
      {/* RIGHT PANEL (map) */}
      <div style={{ flex: 1, border: '1px solid #ddd', borderRadius: '8px' }}>
        <MapComponent
          spatialData={Object.keys(filteredFeatures).length>0?filteredFeatures:spatialData}
          initialCenter={[-6.764538, 39.214464]}
          onBoundsChange={handleBoundsChange}
          layerColors={Object.fromEntries(Object.keys(defaultLayerColors).map(layer=>[layer,getLayerColor(layer)]))}
          highlightedFeatures={filteredFeatures}
        />
      </div>
    </div>
