...
// ... (debounce, fetchWithRetry, checkTokenValidity, useLocalStorageCache helpers stay same)
...
function MapView() {
  ...
  // ------------------------
  const SPATIAL_API_BASE = (import.meta.env.VITE_API_SPATIAL_URL ||
    "https://smds.onrender.com/api/spatial").replace(/\/$/, "");
  ...
  const getLayerColor = useCallback(
    (layer) => customColors[layer] || defaultLayerColors[layer],
    [customColors]
  );

  // ------------------------
  // BBOX Fetch for selected layers
  // ------------------------
  const fetchGeoByBbox = useCallback(
    debounce(async (layers, bounds, simplify = 0.0001) => {
      if (!layers || layers.size === 0 || !bounds) return;
      ...
      for (const layer of layers) {
        try {
          ...
          const fc = resp.data || { type: "FeatureCollection", features: [] };
          newSpatialData[layer] = fc.features;
        } catch (err) {
          console.warn("Layer failed:", layer, err);
          newSpatialData[layer] = [];
        }
      }
      setSpatialData(newSpatialData);
  ...
  return (
    <div style={{ display: "flex", height: "100vh" }}>
      {/* Sidebar */}
      <div style={{ width: "250px", background: "#fff", padding: "10px" }}>
        <h3>Landbase Layers</h3>
      ...
      <div style={{ flex: 1 }}>
        <MapComponent
          ...
          initialCenter={[-6.163, 35.7516]}
          onBoundsChange={handleBoundsChange}
          layerColors={Object.fromEntries(
            Object.keys(API_ENDPOINTS).map((l) => [l, getLayerColor(l)])
          )}
        />
      </div>
    </div>
