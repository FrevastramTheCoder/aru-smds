...
function SystemDashboard() {
  ...
  // Fetch per-category data with staggered delay
  const fetchCategoryData = useCallback(async (categoryKey, delayMs = 0) => {
    ...
    try {
      const res = await fetchWithRetry(`${SPATIAL_API_BASE}/geojson/${categoryKey.toLowerCase().replace(/\s+/g, '-')}`, {
  ...
  // Fetch all categories with staggered requests
  const fetchAllData = useCallback(() => {
    DASHBOARD_CATEGORIES.forEach((c, idx) => {
      fetchCategoryData(c.key, idx * 500); // 0.5s stagger per category
    });
  }, [fetchCategoryData]);
