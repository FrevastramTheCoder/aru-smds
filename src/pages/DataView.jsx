...
function DataView() {
  ...
  // Redirect to a default page or show error if layerName is undefined
  useEffect(() => {
    if (!layerName) {
      toast.error("No layer specified. Redirecting to home page...");
      setTimeout(() => navigate("/"), 2000);
    }
  }, [layerName, navigate]);

  const fetchData = useCallback(async (page = 1, elementType = "all") => {
    if (!layerName || !token) {
      setError("Missing layer name or authentication token");
      setLoading(false);
      return;
    }
  ...
  if (!layerName) {
    return (
      <div style={containerStyle}>
        ...
        <p style={errorStyle}>
          No layer specified in the URL. Please select a layer from the data management page or ensure the route includes a valid layer name.
        </p>
      </div>
    );
  }
    ...
    <div style={containerStyle}>
      ...
      {error && (
        <div style={errorStyle}>
          {error}
          <br />
          <button onClick={() => navigate("/")} style={buttonStyle("#2563eb")}>
            Return to Home
          </button>
        </div>
      )}
