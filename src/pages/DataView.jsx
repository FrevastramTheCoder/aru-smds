...
function DataView() {
  ...
  const currentLayer = layerName || "all"; // Fallback to 'all'
  const [data, setData] = useState([]);
  const [groupedData, setGroupedData] = useState({});
  const [searchTerm, setSearchTerm] = useState("");
  const [loading, setLoading] = useState(false);
  ...
  const fetchData = async (page = 1, elementType = "all") => {
    ...
    try {
      ...
      let url = `${API_BASE}/data/${encodeURIComponent(currentLayer)}?page=${page}&limit=${limit}`;
      if (elementType !== "all") {
      ...
      if (res.data?.success) {
        const rows = res.data.data.map((r) => {
          const attributes = r.attributes || {};
          const geometry = r.geometry || {};
          const elementType = r.elementType || currentLayer || "unknown";
          const style = getLayerStyle(elementType);
          return {
            id: r.id,
            attributes,
      ...
      console.error("[DataView] Error fetching data:", err);
  ...
  useEffect(() => {
    if (token) fetchData(1, selectedElement);
    else toast.error("Please log in to view data");
  }, [currentLayer, limit, token, selectedElement]);
  ...
  // ...All remaining functions (executeQuery, toggleRow, toggleColumn, renderValue, exportCSV/JSON/XLSX, renderElementTable, etc.) remain unchanged
  // Make sure to replace all `layerName` references for API calls or exports with `currentLayer` to handle the fallback case

  return (
    <div style={{ maxWidth: "95%", margin: "40px auto", padding: 20 }}>
      <ToastContainer />
      <h1 style={{ fontSize: 32, fontWeight: "bold", marginBottom: 20, textAlign: "center" }}>
        Data View - {dataTypes.find((dt) => dt.key === currentLayer)?.label || currentLayer}
      </h1>

      {/* The rest of your UI components remain the same */}
    </div>
  );
}
